...
async function clearTmpDirectory() {
    try {
        ...
        if (!fs.existsSync(tmpDir)) {
            return { success: false, message: 'Temporary directory does not exist‼️' };
        }
        ...
        if (files.length === 0) {
            return { success: true, message: 'Temporary directory is already empty‼️' };
        }
    ...
    try {
        ...
        if (!isOwner) {
            await sock.sendMessage(chatId, { 
                text: '❌ This command is only available for the owner‼️' 
            });
            return;
        }
        ...
        await sock.sendMessage(chatId, { 
            text: '❌ Failed to clear temporary files‼️' 
        });
    }
}
