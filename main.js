...
const groupCreator = require('./commands/creategroup');
...
const videoCommand = require('./commands/v;/antibug');
...
const getnewsletterCommand = require('./commands/getnewsletter');
const helpCommand = require('./commands/help');
const bioCommand = require('./commands/bio');
const banCommand = require('./commands/ban');
const antigroupmentionCommand = require('./commands/antigroupmention');
const { checkGroupMentions } = require('./lib/antigroupmention');
const { promoteCommand } = require('./commands/promote');
const { demoteCommand } = require('./commands/demote');
const { handleIncomingCall, anticallCommand } = require('./commands/anticall');
const muteCommand = require('./commands/mute');
const autoreactCommand = require('./commands/autoreact');
const { handleAutoReact } = require('./lib/autoreact');
...
const { autoStatusCommand, handleStatusUpdates } = require('./commands/autostatus');
...
global.channelLink = "https://whatsapp.com/channel/0029Vb5iurcFsn0g8SxxBs0p";
global.ytch = "ZUKO";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // Check for spam
const isSpam = await handleAntiSpam(sock, message);
if (isSpam) {
    await sendMessage(chatId, {
        text: '⚠️ Your message was flagged as spam',
        ...channelInfo
    });
    return;
}
...
const isExcessiveMention = await handleMentions(sock, message);
if (isExcessiveMention) return;
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.vcf'):
                await vcfCommand({ sock, chatId, message, args: userMessage.split(' ').slice(1) });
                break;
                case userMessage === '.listonline' || userMessage === '.online':
    await listOnlineCommand(sock, chatId, message);
    break;
                case userMessage.startsWith('.getpp'):
    await getppCommand(sock, chatId, message);  // Fixed chatid to chatId
    break;
                case userMessage === '.toaudio' || userMessage === '.mp3':
    await toaudioCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.bio'):
                await bioCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.antibug'):
                await antibugCommand(sock, chatId, message);
                break;
    ...
    await antispamCommand(sock, chatId, message, userMessage.split(' ').slice(1));
    break;
            case userMessage.startsWith('.attp'):
                await attpCommand(sock, chatId, message);
                break;
            case userMessage === '.time' || userMessage === '.now':
    ...
    await antigroupmentionCommand.handle(sock, chatId, message, userMessage.split(' ').slice(1));
    break;
    ...
    await hackCommand.handle(sock, chatId, message, userMessage.split(' ').slice(1));
    break;
    ...
    if (isNaN(position)) {
        ...
        await handleTicTacToeMove(sock, chatId, senderId, position);  // Replaced tictactoeMove with handleTicTacToeMove
    }
    break;
            ...
            case userMessage.startsWith('.compliment'):
                ...
                break;
            case userMessage === '.getnewsletter' || userMessage === '.newsletterjid':
    await getNewsletterCommand(sock, chatId, message);
    break;
    ...
    break;
                case userMessage === '.typing' || userMessage === '.autotyping':
    ...
    break;
            ...
            case userMessage === '.resetlink' || userMessage === '.revoke' || userMessage === '.anularlink':
                ...
                break;
             case userMessage === '.autorecording' || userMessage === '.recording':
    ...
    await autoreactCommand.handle(sock, chatId, message, userMessage.split(' ').slice(1));
    break;
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        if (action === 'add') {
            ...
            const channelId = welcomeData?.channelId || '120363401680775438@newsletter';
        ...
        if (action === 'remove') {
            ...
            // Handle auto-reaction to messages
if (isGroup) {
    await handleAutoReact(sock, message);
            ...
            const channelId = goodbyeData?.channelId || '120363401680775438@newsletter';
...
// Export the handlers
// At the bottom of the file, ensure the export matches:
module.exports = {
    handleMessages,
    handleGroupParticipantUpdate,
    handleStatus: handleStatusUpdates  // Matches the imported name
};